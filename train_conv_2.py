...
VERSIONS = ["v0.6.2.test", "v0.6", "v0.5"]
...
class TrainingAgent():
    ...
    def __init__(self):
        ...
        self.games_per_generation = 100 # CHANGE BACK
        ...
        self.games_per_evaluation = 10 # CHANGE BACK
    ...
    def starting_model(self):
        ...
        def widen(x):
            K[]
             = Input(shape=(6, 6, 3, 3), name='state_input')

        state_input = Input(shape=(6, 6, 3, 3), name='state_input')
        
        widen = Lambda(lambda x: x[:, side, new_x, new_y] * )
        conv = Conv3D(64, kernel_size=3, 
                          strides=(0, 1, 1), 
                          padding='same', 
